...
import com.swf.playground.controller.HelloController;
...
class HelloControllerTest {
	...
	public void testHelloPage() throws Exception {
		RequestBuilder request = MockMvcRequestBuilders.get("/").accept(MediaType.TEXT_PLAIN); // .contentType()
																								// also
																								// available
		this.mvc.perform(request).andExpect(status().isOk()).andExpect(status().is(200))
				.andExpect(content().string("Hello friend."));
	}

	@Test
	public void testIndexAcceptsQueryString() throws Exception {
		this.mvc.perform(MockMvcRequestBuilders.get("/?one=1&two=2")).andExpect(status().isOk());
	}
	...
	public void testMathPi() throws Exception {
		...
		this.mvc.perform(MockMvcRequestBuilders.get("/math/pi")).andExpect(status().isOk())
